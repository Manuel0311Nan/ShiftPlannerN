...
export function PlantillaEditor() {
  ...
  function agregarBloque(diaSemana: string) {
    setBloques((prev) => [
      ...prev,
      { diaSemana, nombre: "Turno", horaInicio: "09:00", horaFin: "17:00", personasRequeridas: 1 },
    ]);
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-[14px] text-ink-secondary">Horario semanal del local</p>
            ...
            .map(({ bloque, index }) => (
              <div key={index} className="flex flex-wrap items-center gap-2">
                <input
                  ...
                  placeholder="Nombre"
                  className="w-28 rounded-xs border border-[#dddddd] px-2 py-1 text-[14px]"
                />
                <input
                  type="time"
                ...
                <input
                  type="number"
                  min={1}
                  value={bloque.personasRequeridas}
                  onChange={(e) =>
                    actualizarBloque(index, { personasRequeridas: Number(e.target.value) })
                  }
                  className="w-16 rounded-xs border border-[#dddddd] px-2 py-1 text-[14px]"
                  title="Personas requeridas"
                />
                <button
                  type="button"
      ...
      <input type="hidden" name="plantilla" value={JSON.stringify(bloques)} readOnly />
    </div>
  );
}
